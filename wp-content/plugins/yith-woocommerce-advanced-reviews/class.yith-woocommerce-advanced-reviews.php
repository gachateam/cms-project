...
if ( ! class_exists( 'YITH_WooCommerce_Advanced_Reviews' ) ) {
	...
	class YITH_WooCommerce_Advanced_Reviews {
		 ...
		 */
		public $meta_key_review_author_IP = YITH_YWAR_META_REVIEW_AUTHOR_IP;
...
// Registering your Custom Post Type
		...
		public function update_tab_reviews_count( $tabs ) {
			...
			if ( isset( $tabs['reviews'] ) ) {
				$product_id               = yit_get_prop( $product, 'id' );
				$tabs['reviews']['title'] = sprintf( esc_html__( 'Reviews(%d)', 'yith-woocommerce-advanced-reviews' ), count( $this->get_product_reviews_by_rating( $product_id ) ) );
			}
		...
		public function add_plugin_metabox() {
			add_meta_box( 'reviews-metabox', esc_html__( 'Review attributes', 'yith-woocommerce-advanced-reviews' ), array(
				$this,
		...
		public function get_product_rating_html( $rating_html, $rating ) {
			...
			if ( $rating > 0 ) {
				...
				$rating_html = '<div class="star-rating" title="' . sprintf( esc_html__( 'Rated %s out of 5', 'yith-woocommerce-advanced-reviews' ), $rating ) . '">';
